...
// This is a wall, floor and landmark spawner object that will generate these objects based on the config
namespace WallSystem
{
    public class GenerateWall : MonoBehaviour
    {
        public bool MakeFloor = true;
        public GameObject Wall; // This is the wall prefab that represents the walls
        public GameObject Generator; // This is object that generates pickups.
        private List<GameObject> _created; // List of objects that GenerateWall has created --> keep track to garbage collect
        ...
        private void Start()
        {
            ...
            {
                Color col = Misc.GetColour(E.Get().CurrTrial.enclosure.GroundColor);
        ...
        {
            Color col = Misc.GetColour(E.Get().CurrTrial.enclosure.WallColor);
            MakeFloor = true;
        ...
        private void GenerateLandmarks()
        {
            foreach (var p in E.Get().CurrTrial.trialData.LandMarks)
            {
                ...
                // landmark.AddComponent<RotateBlock>(); // TODO: ???
                
                landmark.transform.localScale = d.ScaleVector;
                
                try {landmark.transform.position = d.PositionVector;}
                catch (Exception _) {landmark.transform.position = new Vector3(d.PositionVector.x, 0.5f, d.PositionVector.z);}
        ...
        private void GenerateWalls()
        {
            ...
            GameObject.Find("Ground").transform.position = new Vector3(
                E.Get().CurrTrial.enclosure.Position[0], 
                0, 
                E.Get().CurrTrial.enclosure.Position[1]
            );

            GameObject.Find("Ground").AddComponent<BoxCollider>();
            BoxCollider roomCollider = GameObject.Find("Ground").GetComponent<BoxCollider>();
            roomCollider.center = new Vector3(
                GameObject.Find("Ground").transform.position.x,
                1.5f,
                GameObject.Find("Ground").transform.position.z
            );
            roomCollider.size = new Vector3(7.8f, 3f, 7.8f); // 8 = Right on the walls of the enclosure
