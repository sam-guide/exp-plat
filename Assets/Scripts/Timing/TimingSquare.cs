...
namespace Assets.Scripts.Timing
{
    public class TimingSquare : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            Debug.Log("Timing Status: " + Config.DataSingleton.GetData().TimingVerification);
            ...
            if (Config.DataSingleton.GetData().TimingVerification)
            {
                timingBox.enabled = true;
                if (Config.DataSingleton.GetData().TrialInitialValue % 2 == 0)
                {
